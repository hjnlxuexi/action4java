...
public abstract class ContainerBase extends LifecycleMBeanBase
    ...
    /**
     * startStopExecutor在init阶段创建，但是coreSize = maxSize = 1，只有一个线程工作
     * 可以通过设置server.xml中Container相关标签的startStopThreads属性，来调用Container.setStartStopThreads(int startStopThreads)改变线程数量
     */
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        // 启动子容器
        Container[] children = findChildren();//先拿到子容器数组
        List<Future<Void>> results = new ArrayList<>();
        for (int i = 0; i < children.length; i++) {
            // 遍历所有子容器
            // 利用 线程池 + Future 的方式 执行所有子容器的start()
            // 内部子容器会逐层递归的调用start()，StandardHost.start()
            results.add(startStopExecutor.submit(new StartChild(children[i])));
        }

        // 遍历Future，get() 确保所有的子容器都已经start完成
        ...
        // 启动Pipeline
        // Pipeline是管道组件，用于封装了一组有序的Valve(阀门)，便于Valve顺序地传递或者处理请求
    ...
    protected class ContainerBackgroundProcessor implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                while (!threadDone) {
                    try {
                        // 每 backgroundProcessorDelay 秒 执行一次
