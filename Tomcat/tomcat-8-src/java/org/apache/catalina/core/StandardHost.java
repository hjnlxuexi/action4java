    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        if ((errorValve != null) && (!errorValve.equals(""))) {
            try {
                ...
                // 如果未找到则添加到 Pipeline 中，注意：在往 Pipeline 中添加 Valve 阀门时，是添加到 first 后面，basic 前面
        ...
        /*
         * 在 start 的时候，StandardHost 并没有做太多的处理，那么 StandardHost 又是怎么知道它有哪些 child 容器需要启动呢？
         *
         * tomcat 在这块的逻辑处理有点特殊，使用 HostConfig 加载子容器，
         * 而这个 HostConfig 是一个 LifecycleListener，它会处理 start、stop 事件通知，
         * 并且会在线程池中启动、停止 Context 容器
         */
    }
