...
public class Catalina {
     ...
     */
    protected Server server = null;//持有Server的引用，便于后面的：load、start、stop等传递性操作
    ...
    public void load() {
        ...
        // Create and execute our Digester
        //定义解析server.xml的配置，告诉Digester那个xml标签解析成什么类，Server-->Service-->Engine...等等
        ...
        try {
            ...
            // conf/server.xml 和 server-embed.xml都没找到，则直接return
            ...
            try {
                ...
                // 把Catalina作为栈顶实例
                digester.push(this);
                // 解析各个组件：Server、Container、Connector......
        ...
        // 这个设计流弊，
        // 1、用Catalina执行加载工作，
        // 2、在加载工程中产生Server实例，Catalina持有Server实例
        // 3、通过Catalina继续完成Server及后续的其他组件初始化工作
        ...
        try {
            //调用LifeCycle的init阶段，进行初始化
            //并逐层初始化下面的组件，Service，Engine，Connector
            //但是 Engine容器下面的子容器：Host、Context、Wrapper 都在start阶段初始化。
            //          因为这些组件和具体的webapp关联，非常耗时，在start阶段利用多线程完成
            getServer().init();
        } catch (LifecycleException e) {
    ...
    public void start() {
        ...
        // 检查一下Server有没有被初始化
        // 没有，则重新执行load工作
        // 也就是说，在外部调用时，可以直接使用start即可，因为内部已经包含了load过程
        if (getServer() == null) {
            load();
        }
        ...
        // Bootstrap中会设置await为true，其目的在于：开启shutdown端口的监听并阻塞，用于监听关闭指令
        if (await) {
            await();
            stop();
        }
