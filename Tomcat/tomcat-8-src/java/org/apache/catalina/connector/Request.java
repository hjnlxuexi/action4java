...
public class Request implements org.apache.catalina.servlet4preview.http.HttpServletRequest {
     ...
     *
     *
     * tomcat 为了能够重用Request对象
     * 在每次请求结束之后，都会把Request对象内部的引用全部请停掉
     * 包括Session信息，Cookie信息等
     *
     */
    public void recycle() {
    ...
    protected Session doGetSession(boolean create) {
        ...
        // 如果request sessionId存在，则尝试获取Session，拿到了则返回
        if (requestedSessionId != null) {
            try {
                session = manager.findSession(requestedSessionId);
            } catch (IOException e) {
